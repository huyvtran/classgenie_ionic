...
//upload the post 
  ...
  $scope.student_name =Service_Store.getLocal('stud_name_first');
   ...
   if($scope.selection==0 && textmessage==undefined){
    ...
    ajaxloader.loadURL(global.config['api_url']+'/schoolstory/savemsgpost?token='+Service_Store.getLocal('app_token'),
    {
      ...
      sender_ac_no:stored_memberNo
       ...
       var options = new FileUploadOptions();
           ...
           $scope.progressPopup();
        $cordovaFileTransfer.upload(global.config['api_url']+'/schoolstory/post?token=aforetechnical@321!', file_path, options).then(function (result) {
                 $scope.hide();                                
       ...
       console.log("imagepost:: "+ $scope.selection);
       ...
       var params = {};
        ...
        options.chunkedMode = false;
        $scope.progressPopup();
        $cordovaFileTransfer.upload(global.config['api_url']+'/schoolstory/post?token=aforetechnical@321!', imageName, options).then(function (result) {
              // $scope.hide();  
              $scope.progressBar.close();              
               global._alert({template: 'Story added successfully.', dependency:$ionicPopup});
               //$window.location.href = '#/school_story';

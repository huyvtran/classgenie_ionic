...
storySchool.controller('schoolstory_parentCtrl', ['$scope', '$window', '$location', '$ionicPopup', '$ionicPopover', '$state',
     '$ionicModal', '$ionicHistory', '$http', '$ionicLoading', 
     function ($scope, $window, $location, $ionicPopup, $ionicPopover, $state, $ionicModal,$ionicHistory, $http, $ionicLoading) {
        $scope.show = function() {
            $ionicLoading.show({
...
//http://localhost:3000/studentstory/schools/list?member_no=255&token=aforetechnical@321!
...
ajaxloader.load(global.config['api_url']+'/studentstory/schools/list?token='+Service_Store.getLocal('app_token')+"&member_no="+memberNo,
  ...
  if(res1.status == "Success")
    {     
      ...
      Service_Store.setLocal('das_school_id',$scope.school_id);
      console.log("school id::"+ $scope.school_id );
            ...
            ajaxloader.load(global.config['api_url'] + '/schoolstory/allpostschoolstory?token='
                    ...
                    function (resp) {
                        ...
                        if (res1.status == "Success")
                        {
                            ...
                            console.log("hello hi"+$scope.items);
                           ...
                           if(res1.post.length<10){
                            ...
                            //$scope.name=res1.teacher_name[0].name;
    ...
    $scope.doRefresh = function() {
       ...
       $scope.$broadcast('scroll.refreshComplete');
        ...
        $scope.openCommentPage = function () {
            ...
            $scope.teacher_ac_No=this.item.teacher_ac_no;
           
            Service_Store.setLocal('teacher_post_id', storyId);
            Service_Store.setLocal('school_name', schoolname);
            Service_Store.setLocal('st_tac_no', $scope.teacher_ac_No);
